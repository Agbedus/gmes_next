  ...
  return (
    <section className="mt-6">
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          <div className="rounded-3xl border border-au-dark-green/10 bg-white p-4">
            <h3 className="text-sm font-bold text-au-dark-green">{name}</h3>
            ...
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <OverviewCard title="Budget (total)" value={budgetTotal} subtitle={`EU: ${budgetEU}`} icon="account_balance" iconColor="#fff" iconBg="#FDB813" valueClassName="text-2xl font-extrabold text-au-dark-green" />
              <OverviewCard title="Thematic focus" value={Array.isArray(thematic) ? (thematic as unknown as string[]).join(', ') : String(thematic ?? '\u2014')} icon="public" iconColor="#fff" iconBg="#00843D" valueClassName="text-xl font-bold text-au-dark-green" />
              <OverviewCard title="Snapshot" value={(metrics?.['snapshotDate'] as string | undefined) ?? (pd['snapshotDate'] as string | undefined) ?? ((pd['snapshot_date'] as string | undefined) ?? 'mid-2021')} icon="schedule" iconColor="#fff" iconBg="#00843D" valueClassName="text-2xl font-extrabold text-au-dark-green" />
            </div>
              ...
              {Array.isArray(thematic) ? (thematic as unknown as string[]).map((t, i) => (
                <div key={i} className="py-1 flex items-center gap-2">
                  <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-au-green/10 text-au-green" aria-hidden>
                    <IconlyIcon name={iconForTheme(t)} size={16} color="#00843D" />
                  </span>
